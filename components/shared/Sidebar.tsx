              ...
              {sidebaNavLinks.map((link) => {
                ...
                return (
                  <li key={link.route} className={`w-full border-solid`}>
                    <Link className={`flex tracking-wide py-2 px-4 gap-3 text-[.9375rem] max-w-full whitespace-nowrap rounded-full bg-cover transition-all ${isActive ? 'bg-[#F3FCF3] text-[#075D24] fill-current ' : 'text-[#676767]'} ${!isActive && 'hover:bg-[#F3FCF3] hover:text-[#075D24]'}`} href={link.route}>
                    <span
                      className={cn(
